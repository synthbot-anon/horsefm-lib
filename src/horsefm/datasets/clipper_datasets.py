...
class ClipperParamsHelper(clipper.ClipperParams):
    ...
    def relpath(self, filepath: str):
        ...
        if normpath.startswith(self.rootdir2):
            rootdir = "masterfile2/"
            rootlen = len(self.rootdir2)
        elif normpath.startswith(self.rootdir1):
            rootdir = "masterfile1/"
    ...
    def genders(self, candidate, path):
        if candidate in self.known_genders:
            return self.known_genders[candidate]

        if candidate not in self.unknown_genders:
            print("unknown gender", candidate)
            self.unknown_genders.add(candidate)
...
class MlpDialogueParams(ClipperParamsHelper):
    def __init__(self, masterfile_1, masterfile_2):
        super(MlpDialogueParams, self).__init__(
            ...
            genders=clipper_mlp_values.CHARACTER_GENDERS,
...
def mlp_sfx_params(masterfile_1, masterfile_2):
            ...
            for filename in files:
                ...
                if filename.endswith(".flac"):
                    ...
                    audio_files[name] = {
                        ...
                        "source": name.split("~")[1].split("-")[0].strip()
                        if len(parts) > 1
                        else None,
                    }
                else:
    ...
    for entry in os.scandir(f"{masterfile_1}/Sliced Dialogue/Label files"):
        if not entry.is_file():
            continue
        if not entry.name.endswith("_sfx.txt") and not entry.name.endswith(
            "_music.txt"
        ):
            continue
